...
- Go 版本必须是 **1.26.6**：三个 workflow 都用 `go-version-file: backend/go.mod` 取版本，随后硬断言 `go version | grep -q 'go1.26.6'`。升级 Go 时要同时改 `backend/go.mod`、根目录/`backend`/`deploy` 三个 Dockerfile，以及 `backend-ci.yml`（两处）、`release.yml`、`security-scan.yml` 里的版本引用，否则 CI 或发布镜像会继续使用旧标准库。
